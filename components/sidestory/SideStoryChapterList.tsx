...
import { ArrowLeft, Cpu, Lock, FileText, ChevronRight, AlertTriangle, Star, XCircle, Loader2, Database, GitCommit, Sparkle } from 'lucide-react';
...
interface SideStoryChapterListProps {
  ...
  onEnterExtra?: () => void; // New prop for extra directory navigation
  language: Language;
  isLightTheme: boolean;
}

const SideStoryChapterList: React.FC<SideStoryChapterListProps> = ({ volume, onBack, onSelectChapter, onEnterExtra, language, isLightTheme }) => {
  ...
  // Updated to "garbled" text as per image request
  const extraSummary = {
  ...
  return (
        <div className="h-full bg-halftone overflow-y-auto p-4 md:p-12 relative flex flex-col items-center custom-scrollbar pb-32">
             <div className="w-full max-w-3xl relative z-10 animate-fade-in mt-8 md:mt-0">
                        ...
                        return (
                            <Reveal key={chapter.id} delay={index * 50}>
                                <button
                                    ...
                                    className={`w-full flex items-center gap-4 p-4 border transition-all duration-200 group relative overflow-hidden ${itemClass}`}
                                >
                                    ...
                                    <div className="flex-1 text-left relative overflow-hidden">
                                        <div className={`font-bold font-mono text-sm md:text-base uppercase truncate ${isLegacy ? 'glitch-text-heavy tracking-widest opacity-80' : isConstructing ? 'text-emerald-500 glitch-text-heavy' : isGarbled ? 'glitch-text-heavy text-red-500' : ''}`} data-text={t.title}>{t.title}</div>
                ...
                {/* Independent Extra Entry Point - Leads to dedicated directory (Purple/Green Theme) */}
                {hasExtra && (
