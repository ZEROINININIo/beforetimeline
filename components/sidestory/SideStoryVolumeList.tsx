...
import { Folder, Lock, AlertTriangle, HardDrive, VenetianMask, Star, Sparkles, CloudRain, Cpu, Activity, Crown, Radio, Signal } from 'lucide-react';
...
const SideStoryVolumeList: React.FC<SideStoryVolumeListProps> = ({ volumes, onSelectVolume, onOpenCharModal, onOpenTerminal, language, isLightTheme }) => {
  ...
  const terminalText = {
      title: language === 'zh-CN' ? '临时终端' : language === 'zh-TW' ? '臨時終端' : 'TEMP_TERMINAL',
      subtitle: language === 'zh-CN' ? '接收残留信号' : language === 'zh-TW' ? '接收殘留信號' : 'SIGNAL_INTERCEPT',
      status: 'ONLINE'
  };
  ...
  return (
        <div className="h-full bg-halftone overflow-y-auto p-4 md:p-12 relative flex flex-col items-center">
            ...
            <div className="w-full max-w-6xl relative z-10 grid grid-cols-1 md:grid-cols-3 gap-8 px-2 md:px-4 pb-20">
                ...
                {/* TEMPORARY TERMINAL CARD (Special Side Line) */}
                <Reveal delay={volumes.length * 150} className="w-full h-full md:col-span-1">
                    <button
                        onClick={onOpenTerminal}
                        className={`
                            w-full h-64 relative group transition-all duration-300 transform flex flex-col text-left overflow-hidden cursor-pointer
                        `}
                    >
                        <div
                            style={{ clipPath: 'polygon(0 0, 100% 0, 100% 85%, 90% 100%, 0 100%)' }}
                            className={`
                                absolute inset-0 border-2 transition-colors duration-300
                                ${isLightTheme 
                                    ? 'bg-emerald-50/50 border-emerald-600 text-emerald-900 shadow-[0_0_15px_rgba(16,185,129,0.2)] hover:shadow-[0_0_25px_rgba(16,185,129,0.4)] hover:-translate-y-2' 
                                    : 'bg-emerald-950/20 border-emerald-500 text-emerald-400 shadow-[0_0_15px_rgba(16,185,129,0.15)] hover:bg-emerald-900/30 hover:shadow-[0_0_30px_rgba(16,185,129,0.3)] hover:-translate-y-2'
                                }
                            `}
                        >
                                ...
                                {Array.from({ length: 15 }).map((_, i) => (
                                    <div 
                                        ...
                                        style={{
                                            left: `${Math.random() * 100}%`,
                                            top: '-100%',
                                            animation: `dataRain ${1 + Math.random() * 2}s linear infinite`,
                                            animationDelay: `${Math.random() * 5}s`,
                                        }}
                                    >
                                        01
                            ...
                            <div className="p-6 h-full flex flex-col relative z-20">
                                <div className="flex justify-between items-start mb-auto">
                                    <div className="relative">
                                        <Radio size={32} strokeWidth={1} className="text-emerald-500 relative z-10" />
                                        <div className="absolute -top-1 -right-1 w-3 h-3 bg-emerald-400 rounded-full animate-ping opacity-50"></div>
                                    </div>
                                    <div className="text-[10px] font-mono border border-current px-1 font-black opacity-100 bg-emerald-500/20">
                                        SPECIAL
                                    </div>
                                </div>

                                <div className="space-y-1 mt-4">
                                    <h3 className={`font-black text-xl md:text-2xl font-mono uppercase tracking-tight leading-none ${isLightTheme ? 'text-emerald-700' : 'text-emerald-100 drop-shadow-[0_0_5px_rgba(16,185,129,0.8)]'}`}>
                                        {terminalText.title}
                                    </h3>
                                ...
                                <div className="mt-6 pt-4 border-t border-dashed border-current/30 flex justify-between items-end text-[10px] font-mono">
                                    <div className="flex flex-col gap-1">
                                        ...
                                        <span className="flex items-center gap-1">
                                            STATUS: {terminalText.status}
                                        </span>
                                    </div>
                                    <div className="w-16 h-2 bg-current opacity-20 relative overflow-hidden">
                                        <div className="absolute left-0 top-0 h-full bg-current w-full animate-[shimmer_2s_infinite]"></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        {/* Corners */}
                        <div className={`absolute top-0 left-0 w-4 h-4 border-t-2 border-l-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 ${isLightTheme ? 'border-emerald-700' : 'border-emerald-400'}`}></div>
                        <div className={`absolute bottom-0 right-0 w-4 h-4 border-b-2 border-r-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 ${isLightTheme ? 'border-emerald-700' : 'border-emerald-400'}`}></div>
                    </button>
                </Reveal>
            </div>
        </div>
