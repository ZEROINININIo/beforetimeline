      ...
      <div className="my-6 md:my-10 border-l-4 border-fuchsia-600 bg-fuchsia-950/10 font-mono text-xs md:text-sm shadow-[0_0_15px_-3px_rgba(192,38,211,0.2)]">
        ...
        >
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-1 md:gap-3">
             <span className="animate-pulse tracking-widest text-fuchsia-400 text-[10px] md:text-xs">&gt;&gt;&gt; SYSTEM_INTERCEPT // 0000.2_VOID</span>
        ...
        <div className={`transition-all duration-500 ease-in-out overflow-hidden ${isOpen ? 'max-h-[1000px] opacity-100' : 'max-h-0 opacity-0'}`}>
          <div className="p-4 md:p-6 text-fuchsia-100/90 space-y-2 leading-relaxed tracking-wide font-medium bg-black/20 backdrop-blur-sm">
              {lines.map((line, i) => {
                  const cleanLine = line.replace(/0000\.2Void>>/, '').replace(/【插入结束】|【插入結束】|\[INSERTION_END\]/g, '');
...
const ReaderPage: React.FC<ReaderPageProps> = ({ currentIndex, onChapterChange, language, isLightTheme, readerFont }) => {
  ...
  useEffect(() => {
    if (viewMode !== 'reader' || !canAutoAdvance) return;
  ...
  }, [viewMode, canAutoAdvance, isAutoAdvancing]);
  ...
  const renderContent = (text: string) => {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (trimmed.includes('0000.2Void>>')) {
            ...
            inVoidBlock = true;
            voidBuffer = [line];
            if (trimmed.includes('[INSERTION_END]')) { inVoidBlock = false; nodes.push(<VoidLog key={`void-${i}`} lines={[...voidBuffer]} language={language} />); voidBuffer = []; }
            continue;
        }
        if (inVoidBlock) {
            voidBuffer.push(line);
            if (trimmed.includes('【插入结束】') || trimmed.includes('【插入結束】') || trimmed.includes('[INSERTION_END]')) { inVoidBlock = false; nodes.push(<VoidLog key={`void-${i}`} lines={[...voidBuffer]} language={language} />); voidBuffer = []; }
            continue;
        }
